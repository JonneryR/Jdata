...
class DataLoader(object):
	def __init__(self):
		path = 'D:/learningdata/Mlearing/JData/dataset'
		self.df_sku_info = pd.read_csv(path+"/jdata_sku_basic_info.csv")
		self.df_user_info = pd.read_csv(path+"/jdata_user_basic_info.csv")
		self.df_user_action = pd.read_csv(path+"/jdata_user_action.csv")
		self.df_user_order = pd.read_csv(path+"/jdata_user_order.csv")
		self.df_user_comment = pd.read_csv(path+"/jdata_user_comment_score.csv")
		...
		self.df_user_comment['comment_year'] = self.df_user_comment['comment_create_tm'].dt.year
		self.df_user_comment['comment_month'] = self.df_user_comment['comment_create_tm'].dt.month
		self.df_user_comment['comment_day'] = self.df_user_comment['comment_create_tm'].dt.day
...
class Features(object):
	...
	def MakeLabel_(self):
		...
		if self.MakeLabel:
			...
			BuyOrNotLabel_30_101['Label_30_101_BuyNum'] = BuyOrNotLabel_30_101['Label_30_101_BuyNum'].map(lambda x: 1 if(x>=1) else 0)
			# 用户首次下单时间 回归建模
	...
	def MakeFeature_Order_Comment_(self, 
		...
		# 第一次评价时间
		features_temp_ = features_temp_Order_[(features_temp_Order_['cate']==30) | (features_temp_Order_['cate']==101)].\
											groupby(['user_id'])['comment_day'].\
											min().\
											reset_index().\
											rename(columns={'user_id':'user_id','comment_day':BetweenFlag+'comment_day_cate_30_101_firstday'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
		...
		features_temp_ = features_temp_Order_[(features_temp_Order_['cate']!=30) & (features_temp_Order_['cate']!=101)].\
											...
											min().\
											reset_index().\
											rename(columns={'user_id':'user_id','comment_day':BetweenFlag+'comment_day_cate_other_firstday'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
		#################################################################################################################	
		# 最后一次评价时间
		features_temp_ = features_temp_Order_[(features_temp_Order_['cate']==30) | (features_temp_Order_['cate']==101)].\
											...
											max().\
											reset_index().\
											rename(columns={'user_id':'user_id','comment_day':BetweenFlag+'comment_day_cate_30_101_lastday'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
		...
		features_temp_ = features_temp_Order_[(features_temp_Order_['cate']!=30) & (features_temp_Order_['cate']!=101)].\
											...
											max().\
											reset_index().\
											rename(columns={'user_id':'user_id','comment_day':BetweenFlag+'comment_day_cate_other_lastday'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
		#################################################################################################################	
		# 评价当月平均第几天
		features_temp_ = features_temp_Order_[(features_temp_Order_['cate']==30) | (features_temp_Order_['cate']==101)].\
											...
											mean().\
											reset_index().\
											rename(columns={'user_id':'user_id','comment_day':BetweenFlag+'comment_day_cate_30_101_meanday'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
		...
		features_temp_ = features_temp_Order_[(features_temp_Order_['cate']!=30) & (features_temp_Order_['cate']!=101)].\
											...
											mean().\
											reset_index().\
											rename(columns={'user_id':'user_id','comment_day':BetweenFlag+'comment_day_cate_other_meanday'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
		#################################################################################################################	

		# 评价月份数
		features_temp_ = features_temp_Order_[(features_temp_Order_['cate']==30) | (features_temp_Order_['cate']==101)].\
											groupby(['user_id'])['comment_month'].\
											nunique().\
											reset_index().\
											rename(columns={'user_id':'user_id','comment_month':BetweenFlag+'comment_month_cate_30_101_monthnum'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
		...
		features_temp_ = features_temp_Order_[(features_temp_Order_['cate']!=30) & (features_temp_Order_['cate']!=101)].\
											...
											nunique().\
											reset_index().\
											rename(columns={'user_id':'user_id','comment_month':BetweenFlag+'comment_month_cate_other_monthnum'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
		#################################################################################################################
	...
	def MakeFeature_Action_(self,
		...
		features_temp_ = features_temp_Action_[(features_temp_Action_['cate']!=30) & (features_temp_Action_['cate']!=101)].\
											...
											nunique().\
											reset_index().\
											rename(columns={'user_id':'user_id','a_date':BetweenFlag+'a_date_cate_other_nuique'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
		# 继续整加特征
     ...
     # 时间特征 行为时间间隔
		...
		features_temp_ = features_temp_Action_[(features_temp_Action_['cate']==30) | (features_temp_Action_['cate']==101)].\
											...
											min().\
											reset_index().\
											rename(columns={'user_id':'user_id','day':BetweenFlag+'a_day_cate_30_101_firstday'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
		...
		features_temp_ = features_temp_Action_[(features_temp_Action_['cate']!=30) & (features_temp_Action_['cate']!=101)].\
											...
											min().\
											reset_index().\
											rename(columns={'user_id':'user_id','day':BetweenFlag+'a_day_cate_other_firstday'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
		...
		features_temp_ = features_temp_Action_[(features_temp_Action_['cate']==30) | (features_temp_Action_['cate']==101)].\
											...
											max().\
											reset_index().\
											rename(columns={'user_id':'user_id','day':BetweenFlag+'a_day_cate_30_101_lastday'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
		...
		features_temp_ = features_temp_Action_[(features_temp_Action_['cate']!=30) & (features_temp_Action_['cate']!=101)].\
											...
											max().\
											reset_index().\
											rename(columns={'user_id':'user_id','day':BetweenFlag+'a_day_cate_other_lastday'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')    
		...
		features_temp_ = features_temp_Action_[(features_temp_Action_['cate']==30) | (features_temp_Action_['cate']==101)].\
											...
											mean().\
											reset_index().\
											rename(columns={'user_id':'user_id','day':BetweenFlag+'a_day_cate_30_101_meanday'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
		...
		features_temp_ = features_temp_Action_[(features_temp_Action_['cate']!=30) & (features_temp_Action_['cate']!=101)].\
											...
											mean().\
											reset_index().\
											rename(columns={'user_id':'user_id','day':BetweenFlag+'a_day_cate_other_meanday'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
		#################################################################################################################	
		# 行为1总次数
		features_temp_ = features_temp_Action_[((features_temp_Action_['cate']==30) | (features_temp_Action_['cate']==101))&(
                                 features_temp_Action_['a_type']==1)].\
											groupby(['user_id'])['a_num'].\
											sum().\
											reset_index().\
											rename(columns={'user_id':'user_id','a_num':BetweenFlag+'a_type1_num_30_101_sum'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
		...
		features_temp_ = features_temp_Action_[((features_temp_Action_['cate']!=30) & (features_temp_Action_['cate']!=101))&(
                                 features_temp_Action_['a_type']==1)].\
											...
											sum().\
											reset_index().\
											rename(columns={'user_id':'user_id','a_num':BetweenFlag+'a_type1_num_other_sum'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
		        
		#################################################################################################################	
		# 行为2总次数
		features_temp_ = features_temp_Action_[((features_temp_Action_['cate']==30) | (features_temp_Action_['cate']==101))&(
                                 features_temp_Action_['a_type']==2)].\
											groupby(['user_id'])['a_num'].\
											sum().\
											reset_index().\
											rename(columns={'user_id':'user_id','a_num':BetweenFlag+'a_type2_num_30_101_sum'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
     ...
     # 行为2总次数
		features_temp_ = features_temp_Action_[((features_temp_Action_['cate']!=30) & (features_temp_Action_['cate']!=101))&(
                                 features_temp_Action_['a_type']==2)].\
											...
											sum().\
											reset_index().\
											rename(columns={'user_id':'user_id','a_num':BetweenFlag+'a_type2_num_other_sum'})
		self.data_BuyOrNot_FirstTime = self.data_BuyOrNot_FirstTime.merge(features_temp_,on=['user_id'],how='left')
